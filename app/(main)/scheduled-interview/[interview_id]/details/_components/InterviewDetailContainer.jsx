    ...
    <div className='p-5 bg-white rounded-lg mt-5'>
        ...
        <div className='mt-4 flex items-center justify-between lg:pr-52'>
            ...
            <div>
                ...
                <h2 className='flex text-sm font-bold items-center gap-2' > <Calendar className='h-4 w-4'/> {moment(interviewDetail?.created_at).format('MM DD, YYYY')}</h2>
            </div>
            {interviewDetail?.type && <div>
                <h2 className='text-sm text-gray-500'>Type</h2>
                  <h2 className='flex text-sm font-bold items-center gap-2' > 
                  <Clock className='h-4 w-4'/> {JSON.parse(interviewDetail?.type)[0]}</h2>
            </div>
            }
        </div>
