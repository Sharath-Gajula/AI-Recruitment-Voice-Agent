...
function CreateInterview() {
  ...
  const onGoToNext= () =>{
      ...
      if(user?.credits <= 0){
        toast('You have no credits left and Please add your Credits');
        return;
      }
  ...
  return (
    <div className='mt-5 px-10 md:px-24 lg:px-44 xl:px-56'>
      ...
      <Progress value={step * 33} className='my-5' />
      {
        ...
        GotoNext = {()=> onGoToNext()} />
        :step == 2 ? <QuestionList
        formData = {formData} onCreateLink={(interview_id)=> 
      ...
      {/* here the data will floe from form data to next question list */}
    </div>
  );
}
